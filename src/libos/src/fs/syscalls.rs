...
use super::file_ops::{
    ...
    FcntlCmd, FsPath, LinkFlags, StatFlags, UnlinkFlags, AT_FDCWD, UTIME_OMIT,
};
use super::fs_ops;
...
pub fn do_utime(path: *const i8, times_u: *const utimbuf_t) -> Result<isize> {
    ...
    file_ops::do_utimes(AT_FDCWD, path, atime, mtime, 0)?;
    Ok(0)
}
...
pub fn do_futimesat(dirfd: i32, path: *const i8, times_u: *const timeval_t) -> Result<isize> {
    ...
    let (atime, mtime) = file_ops::get_utimes(times)?;
    file_ops::do_utimes(dirfd, path, atime, mtime, 0)?;
    Ok(0)
}
...
pub fn do_utimensat(
    ...
    let (atime, mtime) = file_ops::get_utimes(times)?;
    file_ops::do_utimes(dirfd, path, atime, mtime, flags)?;
    Ok(0)
}
